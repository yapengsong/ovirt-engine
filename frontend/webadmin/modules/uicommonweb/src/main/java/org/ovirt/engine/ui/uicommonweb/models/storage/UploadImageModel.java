...
public class UploadImageModel extends Model implements ICommandTarget {
    ...
    public void autoSelectImageType(final String pathname) {
        if (pathname != null) {
            if (pathname.toLowerCase().endsWith(".qcow2")) { //$NON-NLS-1$
                getVolumeFormat().setSelectedItem(VolumeFormat.RAW);  // TODO COW once it's supported
            } else {
    ...
    private void initiateResumeUpload() {
        ...
        Frontend.getInstance().runAction(VdcActionType.UploadImageStatus, parameters,
                new IFrontendActionAsyncCallback() {
                    @Override
                    public void executed(FrontendActionAsyncResult result) {
                        initiateResumeUpload2(result);
                    }
                }, this);
    }

    private void initiateResumeUpload2(FrontendActionAsyncResult result) {
        UploadImageModel model = (UploadImageModel) result.getState();
    ...
    private void initiateResumeUpload3(FrontendActionAsyncResult result) {
        UploadImageModel model = (UploadImageModel) result.getState();
    ...
    private native void startUpload(Element fileUploadElement, String proxyUri,
        ...
        function sendChunk() {
            ...
            if (xhr == undefined) {
                ...
                // The load event is triggered when xhr has uploaded all the data, whereas readystatechange
                // is triggered when the backend closes the connection.  We want the latter for transferring; see:
                // http://stackoverflow.com/questions/15418608/xmlhttprequest-level-2-determinate-if-upload-finished
                xhr.onreadystatechange = xhrHandle;
                ...
                if (xhr.status == 200 || xhr.status == 204 || xhr.status == 206) {
                    ...
                    if (getUploadStateString() == UploadStates.CLIENT_ERROR) {
                        ...
                        log.INFO('xhrHandle: Finished transfer in ' + elapsed + ' seconds, '
                                + bytesPerSec / 1048576 + ' MB per second');
